<template>
    <div class="container-fluid pl-2 pr-2">
            ...
            <div class="col-lg-4 purchase-products pr-0">
                <div class="card shadow mb-4">
                    <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                        <h6 class="m-0 font-weight-bold text-primary">{{lang.purchase_products}}</h6>
                        <a href="javascript:void(0)" @click="chooseSupplier()" v-if="chooseSupplierBtn" class="btn btn-secondary btn-sm">{{lang.select_supplier}}</a>
                        <a href="javascript:void(0)" @click="chooseSupplier()" v-if="updateSupplierBtn" class="btn btn-warning btn-sm text-white">
                            {{supplier.company_name}} <i class="fa fa-pencil-alt"></i>
                        </a>
                    </div>
                    <div class="card-body border p-0">
                        <div class="selected-products scroll-bar">
                            ...
                            <div class="col-md-12">
                                <table class="table text-right  overflow-hidden">
                                    ...
                                    <tr>
                                        ...
                                        <td></td>
                                        <td>
                                            <input type="number" min="0" v-model.number="summary.paid_amount" class="form-control payment-box">
                                        </td>
                                    </tr>
                                    <tr>
                                ...
                                <a href="javascript:void(0)" @click="submitPurchase()" class="btn btn-primary btn-block">{{lang.save_and_submit}}</a>
                                <a href="javascript:void(0)" @click="clearAll()" class="btn btn-danger btn-block">{{lang.clear}}</a>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="row justify-content-center">
                            <div class="col-md-2 p-1 single-product" v-for="(product, index) in filteredProduct">
                                <div class="item" @click="setPriceAndQty(product.id)">
                                    ...
                                    <div class="price">{{appConfig('app_currency')}} {{product.purchase_price}}</div>
                                </div>
                            </div>
                        </div>
        ...
        <div class="drawer shadow right" v-if="setSupplier" style="width: 400px">
            ...
            <div class="card shadow mb-4">
                <div class="card-header py-3">
                    <h6 class="m-0 font-weight-bold text-primary">{{lang.select_supplier}}</h6>
                </div>
        ...
        computed: {
            totalCartsValue(){
                ...
                this.carts.forEach((cart) => {
                    total += parseInt(cart.total_price);
                });
                return total;
            },
            ...
            currentDue(){
                ...
                return  parseInt(this.totalCartsValue) - parseInt(this.summary.paid_amount);
            },
